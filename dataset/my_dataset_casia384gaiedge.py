...
class CasiaSegmentation(data.Dataset):
    def __init__(self,  transforms=None, txt_name: str = "train.txt"):
        ...
        # 掩码列表
        self.mask_list = []
        lines = map(str.strip, open('/raid/csh/segdataset/mvss_dataset/CASIAv2_A.txt').readlines())
        data = []
        for line in lines:
            temp = line.split()
    ...
    def __getitem__(self, index):
        ...
        if self.transforms is not None:
            ...
            if random.random() < 0.6 and img.size==target.size:
            # if img.size == target.size:
                img = np.array(img)
...
class CasiaSegmentation2(data.Dataset):
    def __init__(self,  transforms=None, txt_name: str = "train.txt"):
        ...
        # 掩码列表
        self.mask_list = []
        self.image_base = '/raid/csh/segdataset/CASIA 1.0 dataset'
        self.mask_base = '/raid/csh/segdataset/casia1groundtruth-master/CASIA 1.0 groundtruth'
        with open(os.path.join(self.mask_base, 'imgmask_pair_1.0.txt')) as f:
            reader = csv.reader(f)
            for row in reader:
                img_realpath = os.path.join(self.image_base, 'Tp', row[0])
                mask_realpath = os.path.join(self.mask_base, row[1])
                self.image_list.append(img_realpath)
                self.mask_list.append(mask_realpath)
        self.transforms = transforms
    ...
    def __getitem__(self, index):
        ...
        if self.transforms is not None:
            ...
            img1,img2,img3, target, h, w = self.transforms(img, target, 512, 512)
            _edgemap = np.array(target).astype(np.float32)
            _edgemap = mask_to_onehot(_edgemap, 2)
            _edgemap = onehot_to_binary_edges(_edgemap, 2, 2)
            edgemask = torch.from_numpy(_edgemap).float()
...
class CasiaSegmentation3(data.Dataset):
    def __init__(self,  transforms=None, txt_name: str = "train.txt"):
        ...
        # 掩码列表
        self.mask_list = []
        lines = map(str.strip, open('/raid/csh/segdataset/mvss_dataset/COVERAGE.txt').readlines())
        data = []
        for line in lines:
            temp = line.split()
...
class CasiaSegmentation4(data.Dataset):
    def __init__(self,  transforms=None, txt_name: str = "train.txt"):
        ...
        # 掩码列表
        self.mask_list = []
        lines = map(str.strip, open('/raid/csh/segdataset/mvss_dataset/Columbia.txt').readlines())
        data = []
        for line in lines:
            temp = line.split()
            ...
            self.image_list.append(
                '/raid/csh/segdataset/mvss_dataset' + sample_path[1:21] + '4cam_splc/' + sample_path[21:])
            self.mask_list.append('/raid/csh/segdataset/mvss_dataset' + mask_path[1:])
        self.transforms = transforms
...
class CasiaSegmentation5(data.Dataset):
    def __init__(self,  transforms=None, txt_name: str = "train.txt"):
        ...
        # 掩码列表
        self.mask_list = []
        lines = map(str.strip, open('/raid/csh/segdataset/mvss_dataset/NIST16.txt').readlines())
        data = []
        for line in lines:
            temp = line.split()
...
class CasiaSegmentation6(data.Dataset):
    def __init__(self,  transforms=None, txt_name: str = "train.txt"):
        ...
        self.image_base = '/raid/csh/segdataset/mvss_dataset/IMD2020_1/img'
        self.mask_base = '/raid/csh/segdataset/mvss_dataset/IMD2020_1/mask'
        with open('/raid/csh/segdataset/mvss_dataset/IMD2020_1/imd2020.txt') as f:
            reader = csv.reader(f)
            for row in reader:
                img_realpath = os.path.join(self.image_base, row[0])
                mask_realpath = os.path.join(self.mask_base, row[1])
                self.image_list.append(img_realpath)
                self.mask_list.append(mask_realpath)
        self.transforms = transforms
...
class CasiaSegmentation7(data.Dataset):
    def __init__(self,  transforms=None, txt_name: str = "train.txt"):
        ...
        # 掩码列表
        self.mask_list = []
        lines = map(str.strip, open('/raid/csh/segdataset/mvss_dataset/DEFACTO12k-test.txt').readlines())
        data = []
        for line in lines:
            temp = line.split()
