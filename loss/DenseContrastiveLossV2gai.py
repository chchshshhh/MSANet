...
class DenseContrastiveLossV2(nn.Module):
    ...
    def sample_anchors_fast(self, dominant_classes, features):
        ...
        present_inds = torch.where(cls_counts >= self.min_views_per_class) #去掉背景,大于最小值5 ([0,...,n-1], [prese   nt class ids])
    ...
    def get_dist_and_classes(self, label: torch.Tensor, scale: int) -> torch.Tensor:
        ...
        self._scale = scale
        if self.label_scaling_mode == 'nn':
            lbl_down = torch.nn.functional.interpolate(label.unsqueeze(1).float(), (scale, scale), mode='nearest')
