...
from utilsloss import DATASETS_INFO, get_rank, printlog
from torch.nn.functional import one_hot
from utilsloss import Logger as Log
...
class DenseContrastiveLossV2(nn.Module):
    def __init__(self):
        ...
        self.num_real_classes = 2
        self.ignore_class = -1
        self.temperature = 0.1
        self.base_temperature = 1.0
        # self.min_views_per_class = 5
        # self.min_views_per_class = 20
        self.min_views_per_class = 20
        self.label_scaling_mode = 'nn'
        self.cross_scale_contrast = True
        self.dominant_mode = 'all'
        self.eps = torch.tensor(1e-10)
        self.metadata = {}
        # self.max_views_per_class = 2500
        # self.max_views_per_class = 5
        self.max_views_per_class = 5
        ...
        # self.anchors_per_image = configs['anchors_per_image'] if 'anchors_per_image' in configs else 50
        # sanity checks
    ...
    def forward(self, label: torch.Tensor, features: torch.Tensor):
        ...
        sampled_features, flag_error = self.sample_anchors_fast(dominant_classes, features)
        sample_labels = torch.zeros(2, dtype=torch.float).cuda()
        sample_labels[1]=1
        # old
        # sampled_features, sampled_labels, flag_error = self.sample_anchors(dominant_classes, features)
        losses = torch.zeros(1, dtype=torch.float).cuda()
        if flag_error:
            loss = features - features
            loss = loss.mean()
        else:
            for i in range(16):
                losses=losses+self.contrastive_loss(sampled_features[:, i, :, :], sample_labels)
            # lossz=torch.cat(losses,dim=0)
            loss=losses/16
        ...
        return loss[0],sampled_features,sample_labels,1
    ...
    def sample_anchors_fast(self, dominant_classes, features):
        ...
        compare = dominant_classes.unsqueeze(-1) == classes_ids.unsqueeze(0).unsqueeze(
            0)  # n, hw, 1 == 1, 1, n_c => n,hw,n_c
        cls_counts = compare.sum(1)  # n, n_c#将每一张图片每一类存在的个数加起来每一张图像总共512个点

        present_inds = torch.where(
            cls_counts >= self.min_views_per_class)  # 去掉背景,大于最小值5 ([0,...,n-1], [prese   nt class ids])
        ...
        # sampled_features = torch.zeros((2, 12, c, views_per_class), dtype=torch.float).cuda()
        sampled_features = torch.zeros((2, 16, c, views_per_class), dtype=torch.float).cuda()
        ...
        for i in range(total_cls):
            ...
            sampled_features[cls_in_batch[i], batch_inds[i]] = features[batch_inds[i], :, sampled_indices_from_cl]

            # sampled_features[i] = features[batch_inds[i], :, sampled_indices_from_cl]
            # sampled_labels[i] = cls_in_batch[i]

        return sampled_features, flag_error
    ...
    def get_dist_and_classes(self, label: torch.Tensor, scale: int) -> torch.Tensor:
        ...
        self._scale = scale
        if self.label_scaling_mode == 'nn':
            lbl_down = torch.nn.functional.interpolate(label.unsqueeze(1).float(), (h // scale, w // scale),
                                                       mode='nearest')
