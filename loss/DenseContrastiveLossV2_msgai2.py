...
from utilsloss import DATASETS_INFO, is_distributed, concat_all_gather, get_rank, to_numpy, printlog
from torch.nn.functional import one_hot
import torch.distributed
import numpy as np
import datetime
from loss.DenseContrastiveLossV2d import DenseContrastiveLossV2 as DCV2
...
class DenseContrastiveLossV2_ms(nn.Module):
    def __init__(self):
        ...
        self.cross_scale_temperature =  0.1
        self.detach_cs_deepest =  False
        self.w_high_low =  1.0
        self.w_high_mid =  1.0
        self.ms_losses = []
        self.cs_losses = []
        ...
        #     printlog(f'defining dcv2 loss at scale {s}')
            setattr(self, f'DCV2_scale{s}', DCV2())
    ...
    def forward(self, label: torch.Tensor, features: list, **kwargs):
        ...
        if self.cross_scale_contrast:
            ...
            if self.detach_cs_deepest:
                ...
                loss_cross_scalees = torch.zeros(1, dtype=torch.float).cuda()
                for i in range(16):
                    loss_cross_scalees = loss_cross_scalees+self.contrastive_loss(feats_ms[0][:,i,:,:], sample_labels, feats_ms[-1][:,i,:,:], sample_labels)
                self.cs_losses.append((loss_cross_scalees/16).detach())

            loss += self.w_high_low * (loss_cross_scalees[0]/16)
            ...
            if len(feats_ms)>2: # hrnet : 4 , s4-s16 , s4-s32 dlv3 : 3 layer1(s4)-layer4(s8), layer1(s4)-layer3(s8)
                ...
                # loss += self.w_high_mid * loss_cross_scale2
                    loss_cross_scalees2 = torch.zeros(1, dtype=torch.float).cuda()
                    for i in range(16):
                        loss_cross_scalees2 = loss_cross_scalees2+self.contrastive_loss(feats_ms[0][:,i,:,:], sample_labels, feats_ms[-2][:,i,:,:], sample_labels)
                    self.cs_losses.append((loss_cross_scalees2/16).detach())
                loss += self.w_high_mid * (loss_cross_scalees2[0]/16)
        return loss
...
# loss=DenseContrastiveLossV2_ms().cuda()
# a=torch.rand(2,96,64,64).cuda()
# b=torch.rand(2,96,32,32).cuda()
# c=torch.rand(2,96,16,16).cuda()
# d=torch.rand(2,96,8,8).cuda()
# label=torch.randint(0,2,(2,256,256)).cuda()
# list=[]
# list.append(a)
# list.append(b)
# list.append(c)
