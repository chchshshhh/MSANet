...
class SparseViT(nn.Module):
    def __init__(self, layers=[5, 8, 20, 7], img_size=224, in_chans=3, s_blocks3=[4, 8, 16, 32], s_blocks4=[8, 16],
        ...
        for i in range(layers[2]):
            block = SABlock(
                dim=embed_dim[2], num_heads=num_heads[2], sparse_size=s_blocks3[i // 5], mlp_ratio=mlp_ratio,
        ...
        for i in range(layers[3]):
            block = SABlock(
                dim=embed_dim[3], num_heads=num_heads[3], sparse_size=s_blocks4[i // 4], mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, qk_scale=qk_scale,
