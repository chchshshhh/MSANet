...
from src.models.extractor.high_frequency_feature_extraction import HighDctFrequencyExtractor
from src.models.extractor.low_frequency_feature_extraction import LowDctFrequencyExtractor
import math
...
class ConvNeXt(timm.models.convnext.ConvNeXt):
    def __init__(self, conv_pretrain=False):
        ...
        if conv_pretrain:
            ...
            self.load_state_dict(torch.load('/raid/csh/TruFor-main/convnext_tiny_1k_224_ema.pth'),
                                 strict=False)
...
class MixVisionTransformer(nn.Module):
    def __init__(self, pretrain_path=None, img_size=384, patch_size=4, in_chans=3, embed_dims=[64, 128, 320, 512],
...
class Mesorch(nn.Module):
    def __init__(self, seg_pretrain_path='/raid/csh/TruFor-main/mit_b3.pth', conv_pretrain='', if_predict_label=False):
        ...
        # 最后调整到512x512大小
        self.resize = nn.Upsample(size=(384, 384), mode='bilinear', align_corners=True)
    ...
    def forward(self, image, mask, label, *args, **kwargs):
        ...
        mask_loss = self.loss_fn(pred_mask, mask.unsqueeze(1).float())
        loss = mask_loss
