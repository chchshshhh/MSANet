...
class MutualNet(nn.Module):
    def __init__(self, BatchNorm=nn.BatchNorm2d, dim=512, num_clusters=32, dropout=0.1):
    ...
    def forward(self, edge_x, region_x):
        ...
        edge_graph1 = self.edge_conv1(edge_graph.unsqueeze(3)).squeeze(3)
        edge_graph2 = self.edge_conv2(edge_graph.unsqueeze(3)).squeeze(3)
        edge_graph3 = self.edge_conv3(edge_graph.unsqueeze(3)).squeeze(3)
        ...
        region_graph3 = self.region_conv3(region_graph.unsqueeze(3)).squeeze(3)

        # CGI
        n_edge_x = self.r2e(edge_graph1, region_graph2, region_graph3, edge_assign)
        edge_x = edge_x + n_edge_x.view(edge_x.size()).contiguous()
        n_region_x=self.r2e(region_graph1, edge_graph2, edge_graph3,edge_assign)
        region_x=region_x+n_region_x.view(region_x.size()).contiguous()
        # torch.cat(edge_x,region_x)
...
#
#
model=MutualNet(dim=64)
a=torch.rand(2,64,96,96)
out1,out2=model(a,a)
print(out1.size())
print(out2.size())
print(out3.size())
