...
def ifa_feat(res, size, stride=1, local=False):
    ...
    h , w = size
    coords = (make_coord((h,w)).cuda().flip(-1) + 1) / 2 #计算网格16*16 (256，2)
    ...
    def __init__(self, ultra_pe=False, pos_dim=40, sync_bn=False, num_classes=19, local=False, unfold=False, stride=1, learn_pe=False, require_grad=False, num_layer=2):
        ...
        if num_layer == 2:
            self.imnet = nn.Sequential(
                nn.Conv1d(98, 96, 1), norm_layer(96), nn.ReLU(),
                nn.Conv1d(96, 96, 1), norm_layer(96), nn.ReLU(),
            )
        elif num_layer == 0:
            self.imnet = nn.Sequential(
                nn.Conv1d(98, 96, 1), norm_layer(96), nn.ReLU(),
...
# model=ifa_simfpn(local=True).cuda()
# a=torch.rand(2,768,8,8).cuda()
# out=model(a,size=(16,16))
# out2=model(out,after_cat=True,size=(16,16))
# print(out2.size())