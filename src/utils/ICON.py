...
class ICE2(nn.Module):
    def __init__(self, num_channels=64, ratio=8):
        super(ICE2, self).__init__()
        self.conv_cross = nn.Conv2d(3 * num_channels, num_channels, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn_cross = nn.BatchNorm2d(num_channels)
        ...
        self.downsample=UpsampleDeterministic(0.5)
        self.initialize()

    def forward(self, in1, in2=None, in3=None, flag=None):
        if in2 != None and in1.size()[2:] != in2.size()[2:]:
            # in2 = F.interpolate(in1, size=in1.size()[2:], mode='bilinear')
            in2=self.upsample(in2)
        else:
            in2 = in1
        if in3 != None and in1.size()[2:] != in3.size()[2:]:
            # in3 = F.interpolate(in3, size=in1.size()[2:], mode='bilinear')
            in3=self.downsample(in3)
        else:
            in3 = in1
...
class PWV(nn.Module):
    def __init__(self, channels):
        ...
        self.fuse1 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=1), nn.Conv2d(64, 64, kernel_size=3, padding=1),
                                   nn.BatchNorm2d(64), nn.ReLU(inplace=True))
        self.fuse2 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=1), nn.Conv2d(64, 64, kernel_size=3, padding=1),
                                   nn.BatchNorm2d(64), nn.ReLU(inplace=True))
        self.fuse3 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=1), nn.Conv2d(64, 64, kernel_size=3, padding=1),
                                   nn.BatchNorm2d(64), nn.ReLU(inplace=True))
