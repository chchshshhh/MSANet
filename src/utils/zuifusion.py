...
from src.utils.zoomnet import ConvBNReLU

class SAGate2(nn.Module):
    def __init__(self, in_planes, out_planes, reduction=16, bn_momentum=0.0003):
        self.init__ = super(SAGate2, self).__init__()
        self.in_planes = in_planes
        self.bn_momentum = bn_momentum
        ...
        self.trans = nn.Sequential(
            ConvBNReLU(2 * in_planes, in_planes, 1),
            ConvBNReLU(in_planes, in_planes, 3, 1, 1),
            ConvBNReLU(in_planes, in_planes, 3, 1, 1),
            nn.Conv2d(in_planes, 2, 1),
        )
    ...
    def forward(self, rgb, hha):
        ...
        # attention_vector_l = self.gate_rgb(cat_fea)
        # attention_vector_r = self.gate_hha(cat_fea)
        attn = self.trans(cat_fea)
        attn_s, attn_m = torch.softmax(attn, dim=1).chunk(2, dim=1)
        sm = attn_s * rec_rgb + attn_m * rec_hha
