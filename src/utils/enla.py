...
class ENLCA(nn.Module):
    def __init__(self, channel=128, reduction=2, ksize=3, scale=3, stride=1, softmax_scale=10, average=True,
                 conv=default_conv, res_scale=0.1,train=True):
        super(ENLCA, self).__init__()
        self.conv_match1 = BasicBlock(conv, channel, channel // reduction, 1, bn=False, act=None)
        self.conv_match2 = BasicBlock(conv, channel, channel // reduction, 1, bn=False, act=None)
...
if __name__ == "__main__":
    ...
    input=torch.rand(2,768,16,16).cuda()
