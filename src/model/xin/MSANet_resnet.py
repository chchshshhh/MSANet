...
from src.model.xin.resnet_encoder import resnet_encoder
...
class FSP(nn.Module):
    def __init__(self, in_planes, out_planes, reduction=16):
        ...
        self.filter1 = FilterLayer(2*in_planes, out_planes, reduction)
        self.filter2 = FilterLayer(2*in_planes, out_planes, reduction)

    def forward(self, guidePath1,guidePath2, mainPath):
        combined1 = torch.cat([guidePath1,mainPath], dim=1)
        combined2 = torch.cat([guidePath2,mainPath],dim=1)
        channel_weight1 = self.filter1(combined1)
        channel_weight2 = self.filter2(combined2)
        out = mainPath + channel_weight1 * guidePath1+ channel_weight2* guidePath2
        return out

class SAGatezai(nn.Module):
    def __init__(self, in_planes, out_planes, reduction=16, bn_momentum=0.0003):
        self.init__ = super(SAGatezai, self).__init__()
        self.in_planes = in_planes
        self.bn_momentum = bn_momentum

        self.fsp_rgb = FSP(in_planes, out_planes, reduction)
        self.fsp_flip = FSP(in_planes, out_planes, reduction)
        self.fsp_fre = FSP(in_planes,out_planes,reduction)
        self.cross_conv = nn.Conv2d(64 * 3, 64, 1, padding=0)
    ...
    def forward(self, rgb, hha,fre):
        # rgb, hha = x
        b, c, h, w = rgb.size()

        rec_rgb = self.fsp_rgb(hha,fre, rgb)
        rec_flip = self.fsp_hha(rgb,fre, hha)
        rec_fre = self.fsp_fre(rgb,hha,fre)
        cat_fea = self.cross_conv(torch.cat([rec_rgb, rec_flip,rec_fre], dim=1))
...
class SCA(nn.Module):
    def __init__(self):
        ...
        self.squeeze_fre = nn.AdaptiveAvgPool2d(1)
        self.channel_attention_fre = nn.Sequential(
            nn.Conv2d(64, 64, 1, padding=0),
            nn.ReLU(inplace=True),
            nn.Sigmoid())
    ...
    def forward(self, x3_r,x3_d,x3_f):
        ...
        SCA_f_ca = self.channel_attention_fre(self.squeeze_fre(x3_f))
        SCA_3f_o = x3_f *SCA_f_ca.expand_as(x3_f)

        Co_ca3 = torch.softmax(SCA_ca + SCA_d_ca+SCA_f_ca,dim=1)

        SCA_3_co = x3_r * Co_ca3.expand_as(x3_r)
        SCA_3d_co= x3_d * Co_ca3.expand_as(x3_d)

        SCA_3f_co = x3_f * Co_ca3.expand_as(x3_f)

        CR_fea3_rgb = SCA_3_o + SCA_3_co
        CR_fea3_d = SCA_3d_o + SCA_3d_co
        CR_fea3_f=SCA_3f_o+SCA_3f_co


        CR_fea3 = torch.cat([CR_fea3_rgb,CR_fea3_d,CR_fea3_f],dim=1)
...
class TransLayer2r(nn.Module):
    def __init__(self, out_c):
        ...
        self.c3_down = nn.Sequential(ConvBNReLU(256, out_c, 3, 1, 1))
...
class FcnNet(nn.Module):
    def __init__(self,
        ...
        backbone1 = resnet_encoder()
        # backbone2 = uniformer_small(pretrained=True)
        backbone3 = resnet_encoder()
        ...
        self.translayer1 = TransLayer2r(out_c=64)
        self.translayer2 = TransLayer2r(out_c=64)
        self.translayer3 = TransLayer2r(out_c=64)
        ...
        self.siu1 = SIUonly6(64)
        self.siu2 = SIUonly6(64)
        self.siu3 = SIUonly6(64)
        self.siu4 = SIUonly6(64)
    ...
    def forward(self, x1: torch.Tensor,x2,x3, hh=384, ww=384,t=True) -> Dict[str, torch.Tensor]:
        ...
        # x2=self.fad2(x2)
        x3=self.fad2(x3)
        xl1,xl2,xl3,xl4 = self.backbone1(x1)
        xm1,xm2,xm3,xm4 = self.backbone1(x2)
        xh1,xh2,xh3,xh4 = self.backbone3(x3)
...
if __name__ == "__main__":
    ...
    model.to(device)
    input = torch.zeros((1, 3, 384, 384)).to(device)
    flops, params = profile(model.to(device), inputs=(input, input, input))
