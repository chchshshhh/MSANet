...
class RandomRotationLayer(nn.Module):
    ...
    def forward(self, x):
        # 随机生成旋转角度
        angle = (torch.rand(1).item() * 2 - 1) * self.max_angle  # 在[-max_angle, max_angle]范围内随机旋转
        
        # 计算旋转矩阵
        theta = torch.tensor([[math.cos(math.radians(angle)), -math.sin(math.radians(angle)), 0],
                              [math.sin(math.radians(angle)), math.cos(math.radians(angle)), 0]])
        theta = theta.expand(x.shape[0], *theta.shape).to(x.device)  # 扩展到batch维度
...
class FcnNet(nn.Module):
    def __init__(self,
        ...
        self.rotation_angle = 30  # 添加旋转角度参数，与A.Rotate(limit=30, p=1)保持一致
    ...
    def forward(self, x1: torch.Tensor,x2,x3, h=384, w=384,t=True) -> Dict[str, torch.Tensor]:
        ...
        # 使用逆向旋转变换替换原来的垂直翻转操作
        feat2 = self.siu1(xl1, TF.rotate(xm1, -self.rotation_angle))
        feat3 = self.siu2(xl2, TF.rotate(xm2, -self.rotation_angle))
        feat4 = self.siu3(xl3, TF.rotate(xm3, -self.rotation_angle))
        feat5 = self.siu4(xl4, TF.rotate(xm4, -self.rotation_angle))
