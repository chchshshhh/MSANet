...
from src.model.uniformerpfs import FcnNet
import numpy as np
# from src.swintransformer_model import fcn_transformer
from train_utils.train_and_eval_t import train_one_epoch, evaluate, create_lr_scheduler
from dataset.my_dataset_irl384 import CasiaSegmentation,CasiaSegmentation2,CasiaSegmentation3,CasiaSegmentation4
...
class SegmentationPresetTrain:
    ...
    def __call__(self, img, target):
        out=self.transforms(image=np.array(img), mask=np.array(target))
        image=out['image']
        mask=out['mask']
        return (image,mask)
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #
        if epoch > 27:
            # f1 = evaluate(model, val_loader1, device=device, num_classes=num_classes)
            # print(f"f1: {f1:.3f}")
            f2 = evaluate(model, val_loader2, device=device, num_classes=num_classes)
            print(f"f1: {f2:.3f}")
            # # print(f'auc: {auc:.3f}')
        ...
        torch.save(save_file, "pfs2/model-{}.pth".format(epoch))
