...
from loss.DenseContrastiveLossV2_ms import DenseContrastiveLossV2_ms as v2ms
from loss.DenseContrastiveLossV2_msgai import DenseContrastiveLossV2_ms as v2msgai
...
def calculate_pixel_f1val(pd, gt):
    # if np.max(pd) == np.max(gt) and np.max(pd) == 0:
    #     f1, iou = 1.0, 1.0
    #     return f1, 0.0, 0.0
    seg_inv, gt_inv = np.logical_not(pd), np.logical_not(gt)
    ...
    f1 = 2 * true_pos / (2 * true_pos + false_pos + false_neg + 1e-6)
    mcc=((true_pos*true_neg)-(false_pos*false_neg))/((math.sqrt((true_pos + false_pos) * (true_pos + false_neg) * (true_neg + false_pos) * (true_neg + false_neg)))+1e-6)
    precision = true_pos / (true_pos + false_pos + 1e-6)
    recall = true_pos / (true_pos + false_neg + 1e-6)
    iou=true_pos/(true_pos+false_pos+false_neg+1e-6)
    return f1,mcc, precision, recall,iou
...
def criterion3clz4(inputs, target,edgemask):
    ...
    for name, x in inputs.items():
        ...
        if name=='edge' or name=='edge2':
            ...
            losses[name]=0.1*dcv2(target,x)

    # if len(losses) == 1:
        # return losses['out']

    return losses['out'] +losses['x3']+losses['x4']+losses['edge']+losses['edge2']
...
def evaluate(model, data_loader, device, num_classes):
    ...
    with torch.no_grad():
        for image1,image2,image3,target,h,w,edgemask in metric_logger.log_every(data_loader, 1000, header):
            ...
            output1 = model(image1,target,edgemask)
            output1 = output1['pred_mask']
            # output1 = F.interpolate(output1, size=(h[0],w[0]), mode='bilinear', align_corners=False)
            output1=output1
...
def evaluateval(model, data_loader, device, num_classes):
    model.eval()
    # confmat = utils.ConfusionMatrix(num_classes)
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'
    ...
    with torch.no_grad():
        for image1,image2,image3,target,h,w,edgemask in metric_logger.log_every(data_loader, 1000, header):
            ...
            f1,mcc, p, r,iou = calculate_pixel_f1val(output.flatten(), target.flatten())
            z=f1
            z2=mcc
            f1s.append(z)
            mccs.append(z2)
        meanf1 = np.mean(f1s)
        meanmcc = np.mean(mccs)
        # confmat.update(target.flatten(), output.argmax(1).flatten())



    return meanf1,meanmcc
...
def train_one_epoch(model, optimizer, data_loader, device, epoch, num_classes,
    ...
    for (image1,image2,image3, target,edgemask) in metric_logger.log_every(data_loader, print_freq, header):
        ...
        lr = optimizer.param_groups[0]["lr"]
        metric_logger.update(loss=loss.item(), lr=lr)
        for i in range(output['pred_mask'].shape[0]):
            # output11 = F.sigmoid(output['pred_mask'][i])
            output11 = output['pred_mask'][i]
