...
def criterion3clz4(inputs, target,edgemask):
    ...
    # losses['out'] = bcefLoss(inputs['out'], target) + diceloss(inputs['out'], target)
    # losses['con'] = torch.tensor(0.0, dtype=torch.float32, requires_grad=True, device=torch.device('cuda:0'))
    # for i in range(inputs['out'].shape[0]):
    #     losses['con'] =losses['con']+simloss(inputs['back'][i],inputs['fore'][i])
    # print(losses['out'])
    # print(losses['con'])
    # if len(losses) == 1:
        # return losses['out']
